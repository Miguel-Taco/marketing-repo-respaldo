...
public interface RespuestaEncuestaRepository extends JpaRepository<RespuestaEncuesta, Integer> {
        ...
        List<RespuestaEncuesta> findByEncuesta_IdEncuestaAndFechaRespuestaAfter(Integer idEncuesta,
                        java.time.LocalDateTime fechaInicio);

        @Query("SELECT COUNT(rd) FROM RespuestaEncuesta re JOIN re.detalles rd JOIN rd.opcion o WHERE re.encuesta.idEncuesta = :idEncuesta AND o.esAlertaUrgente = true")
        Long countAlertasUrgentes(@Param("idEncuesta") Integer idEncuesta);

        @Query("SELECT COUNT(rd) FROM RespuestaEncuesta re JOIN re.detalles rd JOIN rd.opcion o WHERE re.encuesta.idEncuesta = :idEncuesta AND o.esAlertaUrgente = true AND re.fechaRespuesta >= :fechaInicio")
        Long countAlertasUrgentesAfter(@Param("idEncuesta") Integer idEncuesta,
