...
interface Step2SegmentSelectionProps {
    formData: {
...
export const Step2SegmentSelection: React.FC<Step2SegmentSelectionProps> = ({ formData, onChange, errors }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1 flex flex-col gap-4 border-r border-separator pr-6">
                {/* Search */}
                ...
                {/* Segment List */}
                <div className="flex flex-col gap-2 overflow-y-auto h-96 pr-2">
                    {loadingSegments ? (
                        ...
                        filteredSegments.map((segment) => (
                            <div
                                ...
                                className={`p-3 rounded-lg cursor-pointer transition-all ${formData.idSegmento === segment.id
                                    ? 'border border-primary bg-primary/10'
                                    : 'border border-transparent hover:bg-primary/5'
                                    }`}
                            >
                                <p className={`font-medium text-sm ${formData.idSegmento === segment.id ? 'text-primary' : 'text-gray-800'
                                    }`}>
                                    {segment.nombre}
                                </p>
                            </div>
                ...
                {/* Error message */}
                {errors.idSegmento && (
                    <p className="text-sm text-red-500 mt-2">{errors.idSegmento}</p>
                )}
            </div>

            {/* Right Column - Segment Details */}
            <div className="md:col-span-2 flex flex-col gap-6">
                {loadingDetails && formData.idSegmento ? (
                    ...
                    <>
                        ...
                        <div className="flex items-start gap-2 text-xs text-gray-600 bg-blue-50 p-3 rounded border border-blue-100">
                            ...
                            <p>La campaña se enviará a todos los miembros activos de este segmento.</p>
                        </div>
                    </>
                ) : (
